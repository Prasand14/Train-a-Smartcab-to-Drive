...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False,epsilon=1.0, alpha=0.5):
        ...
        self.n_trials = 0 # setting trials as zero.
        random.seed(24)
    ...
    def reset(self, destination=None, testing=True):
        ...
        if self.testing == True:
            ...
            self.epsilon = float(1)/math.exp(float( a * self.n_trails)
    ...
    def build_state(self):
        ...
        #Checking whether its in learning. And also whether its not present in the Q table.
        #Incase, if its not present, we create a dictionary for the state in the Q-table.
    ...
    def choose_action(self, state):
        ...
        if (not self.learning == True) or pick_random:
            end_ind = len(self.valid_actions) - 1
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning == True:
            if state not in self.Q:
                self.state[Q] = self.default_dictionary()
            ...
            new_q_val = old_q_val + (learn_rate * (reward - old_q_val))
            self.Q[state][action] = new_q_val 
